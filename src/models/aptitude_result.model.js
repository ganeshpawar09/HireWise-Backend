...
const SubTopicAnalysisSchema = new Schema({
  ...
  score: { type: Number, required: true }, // Ensure this is Number, not Double
});
...
const TopicAnalysisSchema = new Schema({
  ...
  score: { type: Number, required: true }, // Ensure this is Number
});

const TestAnalyticsSchema = new Schema({
  totalQuestions: { type: Number, required: true },
  correctAnswers: { type: Number, required: true },
  averageTimePerQuestion: { type: Number, required: true }, // Changed from Double
  ...
  overallScore: { type: Number, required: true }, // Ensure this is Number
});
